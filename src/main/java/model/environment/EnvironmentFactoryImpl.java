...
public class EnvironmentFactoryImpl implements EnvironmentFactory {
    ...
    private String json;
    private Vec3f posListener;
    private Pair<Float, Float> sizeSpace;
    ...
    */
    ...
    public Environment createMonoEnvironment() {
        ...
        try {
            ...
            final JSONArray list = obj.getJSONArray("Mono");
            int i = 0;
    ...
    */
    ...
    public Environment createStereoEnvironment() {
        ...
        try {
            json = Files.readString(Path.of(getClass().getResource(PATH).toURI()));
            final JSONObject obj = new JSONObject(json);
            final JSONArray list = obj.getJSONArray("Stereo");
            int i = 0;
            for (; i < list.length() - 2; i++) {
                final var x = list.getJSONObject(i);
                sh.addSource(sourceFac.createFRSourceWithPosition(new Vec3f(x.getFloat("x"),  x.getFloat("y"), 0f), SourceType.values()[x.getInt("t")]));
    ...
    */
    ...
    public Environment createCinemaEnvironment() {
        ...
        try {
            json = Files.readString(Path.of(getClass().getResource(PATH).toURI()));
            final JSONObject obj = new JSONObject(json);
            final JSONArray list = obj.getJSONArray("Cinema");
            int i = 0;
            for (; i < list.length() - 2; i++) {
                final var x = list.getJSONObject(i);
                sh.addSource(sourceFac.createFRSourceWithPosition(new Vec3f(x.getFloat("x"),  x.getFloat("y"), 0f), SourceType.values()[x.getInt("t")]));
    ...
    */
    ...
    public Environment createHIFIEnvironment() {
        ...
        try {
            json = Files.readString(Path.of(getClass().getResource(PATH).toURI()));
            final JSONObject obj = new JSONObject(json);
            final JSONArray list = obj.getJSONArray("homeHIFI");
            int i = 0;
            for (; i < list.length() - 2; i++) {
                final var x = list.getJSONObject(i);
                sh.addSource(sourceFac.createFRSourceWithPosition(new Vec3f(x.getFloat("x"),  x.getFloat("y"), 0f), SourceType.values()[x.getInt("t")]));
